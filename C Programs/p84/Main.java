...
class Main extends RecursiveAction {
    ...
    public Main(Graph graph, int[] distances, int startNode) {
        this.graph = graph;
    ...
    private void bfs(int node) {
        List<Main> subtasks = new ArrayList<>();

        for (int neighbor : graph.getNeighbors(node)) {
            if (distances[neighbor] == -1) {
                distances[neighbor] = distances[node] + 1;
                subtasks.add(new Main(graph, distances, neighbor));
            }
        }
    ...
    public static void parallelBFS(Graph graph, int[] distances, int startNode) {
        ...
        forkJoinPool.invoke(new Main(graph, distances, startNode));
    }
