...
class Main {
    ...
    public Main(FitnessFunction fitnessFunction) {
    ...
    private double evaluateFitness(Individual individual) {
        ...
        for (Double input : inputs) {
            fitness += Math
                    .abs(fitnessFunction.evaluate(List.of(input)) - evaluateExpression(individual.expression, input));
        }
    ...
    public static void main(String[] args) {
        Main gp = new Main(inputs -> Math.sin(inputs.get(0)) + Math.cos(2 * inputs.get(0)));
