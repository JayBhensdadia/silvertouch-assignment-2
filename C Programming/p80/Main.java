...
class ParallelPrefixSum extends RecursiveTask<int[]> {
    ...
    public ParallelPrefixSum(int[] array) {
        this(array, 0, array.length);
    }

    private ParallelPrefixSum(int[] array, int low, int high) {
        this.array = array;
        this.low = low;
        this.high = high;
    ...
    protected int[] compute() {
        if (high - low <= SEQUENTIAL_THRESHOLD) {
            ...
            int mid = (low + high) >>> 1;
            ParallelPrefixSum leftTask = new ParallelPrefixSum(array, low, mid);
            ParallelPrefixSum rightTask = new ParallelPrefixSum(array, mid, high);
            leftTask.fork();
    ...
    public static int[] parallelPrefixSum(int[] array) {
        ...
        return forkJoinPool.invoke(new ParallelPrefixSum(array));
    }
